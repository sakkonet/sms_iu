<script>
    ...
    $(document).ready(function() {
        ...
        $('table tbody').on('click', 'tr .edit_me', function(e) {
            ...
            var applicable_tables = ["tblAddress", "tblFixed_asset", "tblJournal_transaction", "tblExpense", "tblUsers", "tblBill", "tblIncome", "tblInvoice"];
        ...
        $('table tbody').on('click', 'tr .approve', function(e) {
            e.preventDefault();
            var row = $(this).closest('tr');
            var tbl = row.parent().parent();
            var tbl_id = $(tbl).attr("id");
            ...
            var contro = tbl_id.replace("_inactive", "");
            var controller = contro.replace("tbl", "");
            var url = "<?php echo site_url(); ?>" + controller.toLowerCase() + "/approve";
        ...
        $.each(data_array, function(key, val) {
            $.map($('#' + form + ' [name="' + key + '"]'), function(named_item) {
                if (named_item.type === 'radio' || named_item.type === 'checkbox') {
                    ...
                    if (reg_ex.test(key)) {
                        if (val != null) {
                            var date_val = moment(val, 'YYYY-MM-DD').format('YYYY-MM-DD');
    ...
    //Activate function
    function approve(data, url, tblId) {
        msg = "You are about to confirm or Approve the selected record or Transaction. You wont be able to edit or delete. Are you sure you would like to proceed?";
        swal({
                title: "Are you sure?",
                text: msg,
                type: "warning",
