...
<head>
  ...
  <link href="<?php echo base_url("assets/css/jque.css"); ?>" rel="stylesheet">
...
<body class="app">
  ...
  <script>
    $(document).ready(function() {
      ...
      $.ajax({
        url: '<?php echo api_url . "/utils/totalDepleted"; ?>',
        headers: {
          Authorization: 'Bearer ' + '<?php echo $this->session->userdata('auth_token'); ?>'
        },
        dataType: 'json',
        success: function(feedback) {
          if (feedback.success) {
            viewModel.depletedbundles(feedback.data);
      ...
      //This populates both sent messages and failed messages
      $.ajax({
        url: '<?php echo api_url . "/utils/totalsentmessages"; ?>',
        headers: {
          Authorization: 'Bearer ' + '<?php echo $this->session->userdata('auth_token'); ?>'
        },
        dataType: 'json',
        success: function(feedback) {
          if (feedback.success) {
            viewModel.totalsenttoday(feedback.data[0].sent);
            viewModel.totalfailedtoday(feedback.data[0].failed);
